...
getSimMols<-function(input, sim.thres){
  ...
  targets <- left_join(sims2, evo) %>% dplyr::select(Structure_ID, Supplier_Molname, Common_Name, sim) %>% distinct()
  print(targets)
}
...
getNetwork<-function(input, sim.thres){
  ...
  sims2 <- sims %>% filter(sim > sim.thres) %>% arrange(-sim)
  sims2$Structure_ID <- as.character(sims2$match)
  targets <- left_join(sims2, evo) %>% dplyr::select(Structure_ID, sim) %>% distinct()
  targets$from <- "input"
...
getTargetNetwork<-function(input, sim.thres){
  ...
  sims2 <- sims %>% filter(sim > sim.thres) %>% arrange(-sim)
  sims2$Structure_ID <- as.character(sims2$match)
  targets <- left_join(sims2, evo) %>% dplyr::select(Uniprot_accession_numbers, sim) %>% distinct()
  targets$from <- "input"
...
getMolsFromGenes <- function(inp.gene){
  mols <- filter(evo, Hugo_Gene == inp.gene) %>% 
    select(Structure_ID, Supplier_Molname, Common_Name, N_quantitative, N_qualitative, N_inactive) %>% 
    distinct()
  mols
}

getMolsFromGeneNetworks.edges <- function(inp.gene){
  mols <- filter(evo, Hugo_Gene == inp.gene & N_quantitative > 3) %>% 
    select(Structure_ID, N_quantitative) %>% 
    distinct() 
  
  net <- filter(evo, Structure_ID %in% mols$Structure_ID & N_quantitative > 3) %>% 
    group_by(Structure_ID) %>%
    top_n(10, N_quantitative) %>% 
    ungroup()
...
getMolsFromGeneNetworks.nodes <- function(inp.gene){
  ...
  color <- c(rep("blue", length(unique(as.character(net$Structure_ID)))), rep("green", length(unique(as.character(net$Hugo_Gene)))))
